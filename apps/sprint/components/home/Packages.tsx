    ...
    <section id="packages">
      <div className="max-w-6xl mx-auto px-5 sm:px-6 lg:px-8 py-24 sm:py-32">
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-2 gap-5 items-stretch">
          ...
          >
            <div className="relative rounded-2xl border-2 border-stone-900 overflow-hidden h-full flex flex-col bg-white">
              <div className="h-1 bg-gradient-to-r from-brand-500 via-brand-400 to-brand-500" />
              <div className="p-7 sm:p-8 flex flex-col flex-1">
                <div className="flex items-center gap-3 mb-5">
                  <Badge variant="accent">Popular</Badge>
                  <span className="text-[11px] font-semibold text-stone-500 tracking-widest uppercase">2 Weeks</span>
                </div>

                <h3 className="text-xl font-display text-stone-900 tracking-tight">Agency AI Automation Sprint</h3>
                ...
                <div className="mt-6 mb-6">
                  <p className="text-3xl font-bold text-stone-900 tracking-tight font-mono">₹1,50,000</p>
                  <p className="text-[11px] text-stone-500 mt-1 tracking-wide">AED 15,000+ for UAE clients</p>
                </div>

                <ul className="space-y-3 mb-8 flex-1">
                  {['2-3 workflows automated', 'SOP + handover included', 'Dashboard + reporting views', '2-week post-sprint support'].map(item => (
                    <li key={item} className="flex gap-2.5 text-[14px] text-stone-600 leading-relaxed">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <Button href="/government" variant="ghost" size="sm" className="w-full justify-center text-stone-500">
                    Government Variant →
                  </Button>
                </div>
              </div>
          ...
          >
            <div className="rounded-2xl border border-stone-200/60 bg-white shadow-[0_1px_2px_rgba(0,0,0,0.03)] h-full flex flex-col">
              <div className="p-7 sm:p-8 flex flex-col flex-1">
                <div className="flex items-center gap-3 mb-5">
                  <Badge variant="default">Quick Start</Badge>
                  <span className="text-[11px] font-semibold text-stone-500 tracking-widest uppercase">3 Days</span>
                </div>

                <h3 className="text-xl font-display text-stone-900 tracking-tight">Workflow Discovery Sprint</h3>
                <p className="mt-2 text-[14px] text-stone-600 leading-relaxed">
                  Not ready for full implementation? Start with a 3-day discovery to map your top workflow bottleneck.
                </p>

                <div className="mt-6 mb-6">
                  <p className="text-3xl font-bold text-stone-900 tracking-tight font-mono">₹50,000</p>
                  <p className="text-[11px] text-stone-500 mt-1 tracking-wide">AED 2,500+ for UAE clients</p>
                </div>

                <ul className="space-y-3 mb-8 flex-1">
                  {['Process audit of one workflow', 'Bottleneck + risk map', 'Automation opportunity matrix', '14-day implementation roadmap'].map(item => (
                    <li key={item} className="flex gap-2.5 text-[14px] text-stone-600 leading-relaxed">
                      <Check className="w-4 h-4 text-stone-300 flex-shrink-0 mt-0.5" />
                      {item}
                    </li>
                  ))}
                </ul>
