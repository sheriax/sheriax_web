            ...
            >
              <div className="flex-shrink-0">
                <div className={`w-12 h-12 rounded-full flex items-center justify-center text-xs font-bold text-white shadow-md ${
                  i === steps.length - 1 ? 'bg-brand-500' : 'bg-zinc-900'
                }`}>
                  {step.num}
                </div>
              </div>
              <div className="bg-white rounded-xl border border-zinc-200 shadow-[0_1px_3px_rgba(0,0,0,0.06)] p-5 sm:p-6 flex-1">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  <h3 className="font-bold text-zinc-900 text-sm sm:text-base">{step.title}</h3>
                  <span className={`text-[11px] font-semibold rounded px-2 py-0.5 ${
                    i === steps.length - 1 ? 'text-brand-600 bg-brand-50' : 'text-zinc-600 bg-zinc-100'
                  }`}>
                    {step.days}
                  </span>
                </div>
